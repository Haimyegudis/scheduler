...
test('fills 4 stations per shift when enough technicians', () => {
  ...
  const result = generateAssignments([D1], techs, STATIONS);
  expect(result).toHaveLength(8);
  expect(result.filter(a => a.shift === 'morning').map(a => a.stationId).sort()).toEqual(STATIONS);
  expect(result.filter(a => a.shift === 'evening').map(a => a.stationId).sort()).toEqual(STATIONS);
});
...
test('balances total shifts across the week', () => {
  ...
  const result = generateAssignments([D1, D2], techs, STATIONS);
...
test('deterministic: same input, same output', () => {
  ...
  expect(generateAssignments([D1, D2], techs, STATIONS)).toEqual(generateAssignments([D1, D2], techs, STATIONS));
});

test('fills only as many stations as provided, even with surplus technicians', () => {
  const techs = Array.from({ length: 4 }, (_, i) => tech(i + 1, { [D1]: 'morning' }));
  const result = generateAssignments([D1], techs, [21, 22]);
  expect(result).toHaveLength(2);
  expect(result.map(a => a.stationId).sort()).toEqual([21, 22]);
});

test('no stations means no assignments', () => {
  const techs = [tech(1, { [D1]: 'morning' })];
